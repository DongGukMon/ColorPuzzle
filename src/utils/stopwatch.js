...
class StopWatch extends Component {
  ...
  formatTime() {
    ...
    let formatted = formatTimeString(now, msecs).slice(3);

    const splitedForm = formatted.split(':');

    const returnFormat = splitedForm.slice(0, 2).join(':');
    splitedForm[2] = splitedForm[2].slice(0, 2);

    formatted = splitedForm.join(':');
